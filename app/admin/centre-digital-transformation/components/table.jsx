...
const Table = () => {
  ...
  /* ---------------------------------
     FETCH CENTRE DIGITAL TRANSFORMATION SEO
  --------------------------------- */
  const fetchPages = async () => {
    try {
      setLoading(true);
      const res = await api.get(
        "/api/centre-digital-transformation"
      );
      setData(res.data || []);
    } catch (error) {
      console.error(
        "Error fetching centre digital transformation:",
        error
      );
    } finally {
      setLoading(false);
  ...
  const handleEditSave = async (updatedData) => {
    try {
      await api.put(
        `/api/centre-digital-transformation/${editRow._id}`,
        updatedData
      );
