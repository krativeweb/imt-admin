...
const Table = () => {
  ...
  const fetchPages = async () => {
    try {
      ...
      // ✅ PGDM LOGISTICS API
      const res = await api.get("/api/pgdm-logistics-supply-chain");

      setData(Array.isArray(res.data) ? res.data : []);
    } catch (error) {
      console.error(
        "Error fetching PGDM Logistics & Supply Chain:",
        error
      );
      setData([]);
    } finally {
  ...
  const handleEditSave = async (formData) => {
    ...
    try {
      const res = await api.put(
        `/api/pgdm-logistics-supply-chain/${editRow._id}`,
        formData,
        {
  ...
  return (
    <div className="p-2">
      {loading ? (
        ...
        <DataTable
          title="PGDM (Logistics & Supply Chain Management)"
          columns={columns}
          data={filteredData}
          highlightOnHover
