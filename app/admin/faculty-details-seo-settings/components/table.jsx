...
const FacultyDetailsSeoTable = () => {
  ...
  --------------------------------- */
  const fetchFacultyDetailsSeoPages = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/faculty-details-seo");
      setData(res.data || []);
    } catch (error) {
      console.error("Error fetching Faculty Details SEO:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditSave = async (updatedData) => {
    try {
      const res = await api.put(
        `/api/faculty-details-seo/${editRow._id}`,
        updatedData,
        {
      ...
      if (res.data?.data) {
        await fetchFacultyDetailsSeoPages();
        closeEditModal();
      }
    } catch (err) {
      console.error("Faculty Details SEO update failed:", err);
    }
  };
  ...
  return (
    <div className="p-2">
      {loading ? (
        ...
        <DataTable
          title="Faculty Details – SEO Settings"
          columns={columns}
          data={filteredData}
          pagination
          highlightOnHover
          ...
          subHeaderComponent={
            <input
              ...
              placeholder="Search faculty SEO content..."
              style={{ width: "250px" }}
              value={search}
