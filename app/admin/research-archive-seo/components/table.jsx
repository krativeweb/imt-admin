...
const Table = () => {
  ...
  /* ---------------------------------
     FETCH ALL RESEARCH ARCHIVE SEO PAGES
  --------------------------------- */
  const fetchPages = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/research-archive-seo");
      setData(res.data || []);
    } catch (error) {
  ...
  const handleEditSave = async (updatedData) => {
    try {
      const res = await api.put(
        `/api/research-archive-seo/${editRow._id}`,
        updatedData,
  ...
  return (
    <div className="p-2">
      {loading ? (
        ...
        <DataTable
          title="Research Archive SEO Settings"
          columns={columns}
          data={filteredData}
          highlightOnHover
          pagination
